...
## 各省各民族人数
count1=pd.crosstab(students.生源所在地,students.民族,margins=True,margins_name='小计')
count1
count1.to_csv('./ProvinceNation.csv')

## 各省各民族所占百分比
count2=pd.crosstab(students.生源所在地,students.民族,margins=True,margins_name='小计',normalize="all")
count2
count2.to_csv('./PProvinceNation.csv')

#  4. 该校各个专业，各个省，男女分别有多少人，结果写为csv
count3=pd.crosstab([students.专业,students.生源所在地],students.性别)
count3
count3.to_csv('./MajorProvinceGender.csv')
...
dtset1['mean'].apply([np.mean,np.var],axis=1)

#  7. 所有学生的平均分和方差是多少
dtset2=students[['分数1','分数2','分数3']].apply([np.mean,np.var],axis=1)
dtset2
dtset2['mean'].apply([np.mean,np.var])
...
score=pd.read_excel('./在校生.xls','学生成绩单')
del students['scoremean']
alldata=pd.merge(students,score)
alldata
